...
var y k L p w s pi c lambda R u phi i k_hat x g_w y_star k_star;
var L_star p_star w_star s_star c_star c_star lambda_star R_star u_star;
var phi_star i_star k_hat_star x_star_hat g_w_star;

varexo eps_z eps_inv eps_m;

parameters alpha p_index w_index gamma h lambda_p lambda_w L_ss pi beta nu zeta_p zeta_w xi s_pp phi_pi phi_x phi_dx rho_R rho_mp rho_z rho_g rho_mu rho_p rho_w rho_b theta_p theta_w sigma_mp sigma_z sigma_g sigma_mu sigma_p sigma_w sigma_b;

// Model equations in log-linearized form
model;
    // 1. Output gap equation
    y_hat = alpha * k_hat + (1 - alpha) * L_hat;
