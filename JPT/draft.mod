...
// Declare parameters
parameters alpha, delta, ip, iw, gammam, h, lambda_p, lambda_w, L_ss, pi_ss, beta, nu, zeta_p, zeta_w, xi, s2prime, phi_pi, phi_x, phi_dx, rho_R, rho_mp, rho_z, rho_g, rho_mu, rho_p, rho_w, rho_b, theta_p, theta_w,
...
R_ss = (1 + gammam) * pi_ss / beta;
rho_ss = (exp(gammam)/beta)-(1-delta);
...
lambdaL = (cLratio^(-1))*(exp(gammam)-h*beta)/(exp(gammam)-h);
...
c_ss =  cLratio * L_ss;
F    =  FLratio * L_ss;
omega = ((1 / (1 + lambda_p)) * (alpha^alpha) * ((1 - alpha)^(1 - alpha)) * (1 / rho_ss)^alpha)^(1 / (1 - alpha)); // Corrected omega
kappa = ((1 - zeta_p * beta) * (1 - zeta_p)) / (zeta_p * (1 + ip * beta)); // Fixed kappa
...
#den_5 = (exp(gammam) - h * beta) * (exp(gammam) - h);
...
//5.
  lambda = (h * beta * exp(gammam)) / den_5 * c(+1) - (exp(2 * gammam) + (h^2) * beta) / den_5 * c + (h * exp(gammam)) / den_5 * c(-1) + (h * beta * exp(gammam) * rho_z - h * exp(gammam)) / den_5 * z;
//6. 
  lambda = R + lambda(+1) - z(+1) - ppi(+1);
//7. 
  rho = xi * u;
//8. 
  phi = (1 - delta) * beta * exp(-gammam) * (phi(+1) - z(+1)) + (1 - (1 - delta) * beta * exp(-gammam)) * (lambda(+1) - z(+1) + rho(+1));
//9. 
  lambda = phi + mu - exp(2 * gammam) * s2prime * (i - i(-1) + z) + beta * exp(2 * gammam) * s2prime * (i(+1) - i + z(+1));
//10.
  k = u + kbar(-1) - z;
//11. 
...
// Real marginal cost is going to be fixed under flexible economy;
  ...
  lambda_star = (h * beta * exp(gammam) / den_5) * c_star(+1) - ((exp(2 * gammam) + h^2 * beta) / den_5) * c_star + (h * exp(gammam) / den_5) * c_star(-1) + ((h * beta * exp(gammam) * rho_z - h * exp(gammam)) / den_5) * z;

  lambda_star = R_star+lambda_star(+1)-z(+1)-ppi(+1);

  rho_star = xi*u_star;

  phi_star = (1-delta)*beta*exp(-gammam)*(phi_star(+1)-z(+1))+(1-(1-delta)*beta*exp(-gammam))*(lambda_star(+1)-z(+1)+rho_star(+1));

  lambda_star = phi_star+mu-exp(2*gammam)*s2prime*(i_star-i_star(-1)+z)+beta*exp(2*gammam)*s2prime*(i_star(+1)-i_star+z(+1));
